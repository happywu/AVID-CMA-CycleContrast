...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.moco_pretrain = moco_pretrain
    ...
    def _load_moco_checkpoint(self, logger=None):
        """Initiate the parameters from torchvision pretrained checkpoint."""
        ...
        for key in state_dict:
            if 'module.encoder_q' in key:
                new_state_dict[key[len('module.encoder_q.'):]] = state_dict[key]

        # print('loaded keys', new_state_dict.keys())
        ...
        for name, module in self.named_modules():
            if isinstance(module, ConvModule):
                ...
                if 'downsample' in name:
                    ...
                    # layer{X}.{Y}.conv{n}.bn->layer{X}.{Y}.bn{n}
                    original_bn_name = name.replace('conv', 'bn')
                self._load_conv_params(module.conv, new_state_dict,
                                       original_conv_name, loaded_param_names)
                self._load_bn_params(module.bn, new_state_dict,
                                     original_bn_name, loaded_param_names)

        # check if any parameters in the 2d checkpoint are not loaded
        remaining_names = set(
            new_state_dict.keys()) - set(loaded_param_names)
        if remaining_names:
            logger.info(
                f'These parameters in pretrained checkpoint are not loaded'
                f': {remaining_names}')
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            if self.torchvision_pretrain:
                self.pretrained = pretrained
                self._load_torchvision_checkpoint(logger)
            elif self.moco_pretrain:
                self.pretrained = pretrained
                self._load_moco_checkpoint(logger)
            else:
                load_checkpoint(
                    self, pretrained, strict=True, logger=logger)
