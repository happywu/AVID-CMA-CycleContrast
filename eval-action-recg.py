...
parser.add_argument('--ssp-pretrained-weights', default='', type=str)
parser.add_argument('--arch', default='r18', type=str)
parser.add_argument('--from-mmaction', action='store_true')
parser.add_argument('--from-moco', action='store_true')
parser.add_argument('--name', default='moco-r2d50-k400-ep400', type=str)
...
def main_worker(gpu, ngpus, fold, args, cfg):
    ...
    train_loader, test_loader, dense_loader = \
        eval_utils.build_dataloaders(cfg['dataset'],
                                     fold, cfg['num_workers'], args.distributed, logger)
...
def run_phase(phase, loader, model, optimizer, epoch, args, cfg, logger):
    ...
    for it, sample in enumerate(loader):
        ...
        # video: batch x 3 x num_frames x 224x224
        # compute outputs
        if phase == 'test_dense':
