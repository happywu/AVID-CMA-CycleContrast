...
import yaml
import os
from models.video import AV_WrapperR2D
...
def prepare_environment(args, cfg, fold):
    ...
    if args.from_mmaction:
        eval_dir = 'log/{}/eval-{}/fold-{:02d}'.format(args.name, cfg['benchmark']['name'], fold)
    else:
...
def build_dataloader(db_cfg, split_cfg, fold, num_workers, distributed):
    ...
    loader = data.DataLoader(
        db,
        batch_size=db_cfg['batch_size'] if split_cfg['mode'] == 'clip' \
            else max(1, db_cfg['batch_size']//split_cfg['clips_per_video']),
...
class ClassificationWrapper(torch.nn.Module):
    def __init__(self, feature_extractor, n_classes, feat_name, feat_dim, pooling_op=None,
                 use_dropout=False, dropout=0.5, freeze_backbone=False):
...
def build_model(feat_cfg, eval_cfg, eval_dir, args, logger):
    ...
    if args.from_mmaction or args.from_moco:
        pretrained_net = AV_WrapperR2D(args)
    else:
        pretrained_net = models.__dict__[feat_cfg['arch']](**feat_cfg['args'])

    # Load from checkpoint
    if args.ssp_pretrained_weights:
        checkpoint_fn = args.ssp_pretrained_weights
    else:
        checkpoint_fn = '{}/{}/checkpoint.pth.tar'.format(feat_cfg['model_dir'], feat_cfg['name'])
    loaded_pretrained_model = False
    if os.path.exists(checkpoint_fn):
        logger.add_line("load model from {}".format(checkpoint_fn))
        ckp = torch.load(checkpoint_fn, map_location='cpu')
        if args.from_mmaction or args.from_moco:
            pretrained_net.load_state_dict(ckp)
        else:
            msg = pretrained_net.load_state_dict({k.replace('module.', ''): ckp['model'][k] for k in ckp['model']})
            logger.add_line("loaded model {}".format(msg))
    ...
    if loaded_pretrained_model:
        ...
        if args.from_mmaction:
            logger.add_line("File: {}\nEpoch: {}".format(checkpoint_fn, ckp['meta']['epoch']))
        else:
